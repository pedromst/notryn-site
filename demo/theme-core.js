...
try{
 for(const suffix of ['brain','interface-hints','motion','note-side','single-keys','theme','workspace','writing-mode']){
  ...
  if(NotrynDemoStorage.getItem(current)===null&&NotrynDemoStorage.getItem(legacy)!==null)NotrynDemoStorage.setItem(current,NotrynDemoStorage.getItem(legacy));
 }
}catch{}
// Restore workspace hints before paint, independently of keyboard bindings.
try{document.documentElement.dataset.interfaceHints=NotrynDemoStorage.getItem('notryn-interface-hints')==='off'?'off':'on';}catch{}
...
window.NotrynThemes=(()=>{
 const presets=[
  ...
  {id:'jarvis',name:'Jarvis',description:'Holographic glass around a calm reactor core.',bg:'#02070d',panel:'#071923',text:'#d9f7ff',accent:'#48dfff',secondary:'#208acb',colors:['#48dfff','#67aef4','#75ead7','#b4dcff','#58b5d4','#91cfff']},
 ...
 let saved=null;try{saved=NotrynDemoStorage.getItem('notryn-theme');}catch{}
 ...
 function choose(id,omarchy){
  choice=id;saved=id;try{NotrynDemoStorage.setItem('notryn-theme',id);}catch{}
